...
class AdminController extends Controller
{
    ...
    public function thongketheongay(){
        ...
        if(request()->date_from && request()->date_to){
            $time_to = request()->date_from ;
            $new_to = date("Y-m-d", strtotime($time_to));
            $time_form = request()->date_from;
            $new_form = date("Y-m-d", strtotime($time_to));
            $thongke =  $thongke ->whereBetween('orders.date_order',[$new_to,$new_form]);
            
         }elseif (request()->date_from) {
            $time_to = request()->date_from ;
            $new_to = date("Y-m-d", strtotime($time_to));
            $thongke =  $thongke ->where('orders.date_order',$new_to);
         }elseif (request()->date_to) {
            $time_form = request()->date_from;
            $new_form = date("Y-m-d", strtotime($time_to));
            $thongke =  $thongke ->where('orders.date_order',$new_form);
         }
