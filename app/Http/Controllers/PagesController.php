...
class PagesController extends Controller
{
    ...
    function postthongtinthanhvien(Request $rq){
        $this->validate($rq,
            ...
            [
                ...
                'phone.max' => 'Số điện thoại phải có độ dài từ 6-12 kí tự trở lên',
            ]
    ...
    function postdangnhap(Request $rq){
        ...
        [
            'email.required' =>'Bạn chưa nhập email',
            'password.required'=>'Bạn chưa nhập password',
            'password.min'=>'Độ dài kí tự lớn hơn 5 nhỏ hơn 32',
            'password.max'=>'Độ dài kí tự lớn hơn 5 nhỏ hơn 32'
        ]);
    ...
    function postdangki(Request $rq){
        $this->validate($rq,
            ...
            [
                ...
                'address.min' => 'Địa chỉ phải có độ dài từ 6 kí tự trở lên',
                'phone.required' => "Bạn chưa nhập số điện thoại",
                'phone.min' => 'Số điện thoại phải có độ dài từ 6-12 kí tự trở lên',
                'phone.max' => 'Số điện thoại phải có độ dài từ 6-12 kí tự trở lên',
            ]
    ...
    function themgiohangnhieu(Request $request, $id){
        //Kiem tra xem san pham $id da co trong gio hang chua
        $danhsachgiohang = Cart::getContent();

        $checkexists = false;
        foreach ($danhsachgiohang as $ds) {
            if($ds->id == $id){
               $checkexists = true;
               break;
            }
        ...
        if($checkexists){
            Cart::update($id,['quantity' => $request->soluong]);
        } else {
            $product = Product::find($id);
            Cart::add($id, $product->name, $product->discount_price, $request->soluong, array('image' => $product->image));
        }
        return back();
    }
    ...
    function dathang(){
     ...
     function postdathang(Request $rq){
        ...
        // tạo list item for info mail send
        $billMailInfo = array();
        $totalQuanty = 0;
        $totalPrice = 0;
        ...
        foreach ($cart as $key => $value) {
            ...
            $order_detail->save();
            // thêm danh sách item của info mail send
            $info = [
                'id' => $key, 
                'name' => $value->name, 
                'price' => $value->price,
